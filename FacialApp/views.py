...
def ViewRating(request):
    if request.method == 'GET':
       ...
       con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '', database = 'facial',charset='utf8')
       with con:
          cur = con.cursor()
     ...
     if request.method == 'POST' and request.FILES['t3']:
        ...
        if len(faces) > 0:
            ...
            if label == 'angry' or label == 'sad' or label == 'disgust' or label == 'scared' or label == 'surprised':
               output = 'Disappointed'
        print("==================="+output)	
        db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '', database = 'facial',charset='utf8')
